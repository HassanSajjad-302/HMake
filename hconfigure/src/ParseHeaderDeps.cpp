...
struct HeaderDepsParser
{
    ...
    void parseMakeDependencies()
    {
        ...
        for (uint64_t index = 0; index < contents.size(); ++index)
        {
            ...
            if (contents[index] == ':' && (index + 1 == contents.size() || isWhitespace(contents[index + 1])))
            {
                colon = index;
                break;
            }
...
gtl::flat_hash_set<Node *> parseHeaderDeps(std::string &output, const Compiler &compiler, const int exitStatus,
                                           const std::string &dependencyFile, const std::string_view workingDirectory,
                                           const Node *compiledSource, const bool excludeHeadersInConfigureNode)
{
    HeaderDepsParser parser{output, dependencyFile, workingDirectory, compiledSource, excludeHeadersInConfigureNode};
