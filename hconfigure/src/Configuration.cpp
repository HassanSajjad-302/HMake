...
DSC<CPT> &Configuration::GetCPTDSC_P(const string &name_, const string &directory, TargetType targetType)
{
    DSC<CPT> dsc(&(GetCPT()), &(GetPrebuiltLinkOrArchiveTarget(name_, directory, targetType)));
    return const_cast<DSC<CPT> &>(targets<DSC<CPT>>.emplace(dsc).first.operator*());
}

DSC<CPT> &Configuration::GetCPTStaticDSC_P(const string &name_, const string &directory)
{
    DSC<CPT> dsc(&(GetCPT()), &(GetStaticPrebuiltLinkOrArchiveTarget(name_, directory)));
    return const_cast<DSC<CPT> &>(targets<DSC<CPT>>.emplace(dsc).first.operator*());
}

DSC<CPT> &Configuration::GetCPTSharedDSC_P(const string &name_, const string &directory)
{
    DSC<CPT> dsc(&(GetCPT()), &(GetSharedPrebuiltLinkOrArchiveTarget(name_, directory)));
    return const_cast<DSC<CPT> &>(targets<DSC<CPT>>.emplace(dsc).first.operator*());
}
