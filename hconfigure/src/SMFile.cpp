...
void SMFile::initializeBuildCache(const uint32_t index)
{
    ...
    for (const BuildCache::Cpp::ModuleFile::SmRules::SingleHeaderUnitDep &s : smRulesCache.headerUnitArray)
    {
        s.node->toBeChecked = true;
    }
}
...
SMFile *SMFile::findReceivedModule(const N2978::CTBModule &mod) const
{
    SMFile *found = nullptr;
    const string &moduleName = mod.moduleName;
...
void SMFile::sendModule(SMFile &mod)
{
    ...
    if (!allSMFileDependencies.contains(&mod))
    {
        ...
        allSMFileDependencies.emplace(&mod);
    }
...
bool SMFile::build(Builder &builder)
{
    ...
    while (true)
    {
        ...
        if (const auto &r = ipcManager->receiveMessage(buffer, type); r)
        {
            ...
            {
                SMFile *found = findReceivedModule(reinterpret_cast<N2978::CTBModule &>(buffer));
                ...
                if (foundRb.updateStatus != UpdateStatus::JUST_UPDATED)
                {
                    waitingFor = found;
            ...
            else if (type == N2978::CTB::LAST_MESSAGE)
            {
                ...
                if (rb.exitStatus == EXIT_SUCCESS)
                {
                    ...
                    // todo
                    // do header-files, save module and heder-units as well.
                }
    ...
    for (auto &[dependency, ignore] : realBTargets[0].dependencies)
    {
        if (dependency->bTarget->getBTargetType() == BTargetType::SMFILE)
        {
            if (auto *smFile = static_cast<SMFile *>(dependency->bTarget))
            {
                if (allSMFileDependencies.emplace(smFile).second)
                {
                    for (SMFile *smFileDep : smFile->allSMFileDependencies)
                    {
                        allSMFileDependencies.emplace(smFileDep);
                    }
                }
            }
        }
