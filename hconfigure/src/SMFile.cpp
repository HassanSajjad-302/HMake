...
void SourceNode::initializeSourceJson(SourceNode &sourceNode)
{
    initializeSourceJson(sourceNode.sourceJson, sourceNode.node, sourceNode.sourceNodeAllocator, *sourceNode.target);
}
...
void SMFile::updateBTarget(Builder &builder, const unsigned short round)
{
    ...
    if (round == 2 && type == SM_FILE_TYPE::HEADER_UNIT)
    {
        ...
        sourceJson = target->getBuildCache()[Indices::CppTarget::BuildCache::headerUnits][indexInBuildCache];
