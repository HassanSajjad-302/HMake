...
struct CppMod final : CppSrc
{
    ...
    /// thread-safe function that ensures that build-system creates shared-memory bmi file before sending it to the compiler.
    /// if the file is updated, then the compiler creates the mapping. if not then the build-system.
