...
template <MiniTargets = MiniTargets::BASE>
...
template <MiniTargets E0>
template <Dependency dependency, typename T, typename... Property>
LinkOrArchiveTarget<E0> &LinkOrArchiveTarget<E0>::assign(T property, Property... properties)
{
...
template <MiniTargets E0> template <typename T> bool LinkOrArchiveTarget<E0>::evaluate(T property) const
{
    if constexpr (std::is_same_v<decltype(property), Linker>)
    {
...
/*template<>
class LinkOrArchiveTarget2<MiniTargets::MINI> : public LinkOrArchiveTarget2<>
{

};

template<>
class LinkOrArchiveTarget2<MiniTargets::FULL> : public LinkOrArchiveTarget2<>
{

};*/

template <MiniTargets E0>
LinkOrArchiveTarget<E0>::LinkOrArchiveTarget(const pstring &name_, TargetType targetType)
...
template <MiniTargets E0>
LinkOrArchiveTarget<E0>::LinkOrArchiveTarget(bool buildExplicit, const pstring &name_, TargetType targetType)
    : PrebuiltLinkOrArchiveTarget(getLastNameAfterSlash(name_), configureNode->filePath + slashc + name_, targetType,
...
template <MiniTargets E0> void LinkOrArchiveTarget<E0>::setOutputName(pstring outputName_)
{
    actualOutputName = getActualNameFromTargetName(linkTargetType, os, outputName_);
    outputName = std::move(outputName_);
}

template <MiniTargets E0> void LinkOrArchiveTarget<E0>::setFileStatus(RealBTarget &realBTarget)
{
...
template <MiniTargets E0> void LinkOrArchiveTarget<E0>::updateBTarget(Builder &builder, unsigned short round)
{
...
template <MiniTargets E0> LinkerFlags LinkOrArchiveTarget<E0>::getLinkerFlags()
{
    LinkerFlags flags;
...
template <MiniTargets E0> pstring LinkOrArchiveTarget<E0>::getTarjanNodeName() const
{
    pstring str;
    if (linkTargetType == TargetType::LIBRARY_STATIC)
    {
...
template <MiniTargets E0> PostBasic LinkOrArchiveTarget<E0>::Archive()
{
...
template <MiniTargets E0> PostBasic LinkOrArchiveTarget<E0>::Link()
{
...
template <MiniTargets E0> void LinkOrArchiveTarget<E0>::setLinkOrArchiveCommands()
{
    LinkerFlags flags;
...
template <MiniTargets E0> pstring LinkOrArchiveTarget<E0>::getLinkOrArchiveCommandPrint()
{
