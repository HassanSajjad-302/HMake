#ifndef HMAKE_PROJECTCACHE_HPP
#define HMAKE_PROJECTCACHE_HPP
...
template <typename T> T ProjectCache::getOrAddVariable(const string_view name, T defaultValue)
{
    ...
    if (existing != variableLines_.end())
    {
        ...
        if constexpr (std::is_same_v<T, bool>)
        {
            ...
            printErrorMessage(
                FORMAT("A bool cache variable must be exactly true or false.\nVariable: {}\nValue: {}", name, value));
        }
        ...
        else
        {
            if (value.size() < 2 || value.front() != '"' || value.back() != '"')
            {
                printErrorMessage(FORMAT(
                    "A string cache variable must use outermost double quotes.\nVariable: {}\nValue: {}", name, value));
            }
    ...
    lines_.push_back(
        {LineKind::BLANK_OR_COMMENT,
         FORMAT("# Cache variable '{}'. Values are true, false, decimal, or quoted raw strings.", variableName)});
