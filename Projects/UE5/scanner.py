#!/usr/bin/env python3
"""Copy into the UE checkout root and run after UBT's HMake export to generate hmake.cpp."""
...
SHARED_DEFINITIONS_FILE = Path("Engine/Source/HMakeSharedDefs.h")
...
def ubt_definition_arguments(base_command: str, intermediate_directory: Path) -> list[str]:
    """Recover definitions that UBT emits into generated headers rather than the shared command."""
    ...
    if standard is None:
        raise ValueError("UBT baseCommand does not select a C++ standard")

    shared_header = intermediate_directory / "Core" / f"SharedDefinitions.Core.Cpp{standard}.h"
    arguments = read_definition_arguments(shared_header)

    # BuildSettings.cpp also needs these target/toolchain values from its module-specific definitions header.
    build_settings_header = intermediate_directory / "BuildSettings" / "Definitions.h"
    arguments.extend(read_definition_arguments(build_settings_header, {"UE_WITH_DEBUG_INFO", "UE_VFS_PATHS"}))
    return arguments


def generate_shared_definitions(modules: dict[str, dict[str, str]]) -> str:
    ...
    if not macros:
        raise ValueError("UBT HMake export contains no modules")

    lines = [
        "// Generated by scanner.py. Do not edit.",
        "#pragma once",
        "",
...
def compile_commands(
    ...
    if not arguments:
        raise ValueError("UBT baseCommand is empty")
    ...
    try:
        ...
        raise ValueError("UBT baseCommand does not select a source language with -x") from error
    if language_index == len(c_arguments) or c_arguments[language_index] != "c++":
        raise ValueError("UBT baseCommand does not select the expected C++ language mode")
...
def verify_ue_specifications(files: list[UeFile], modules: dict[str, dict[str, str]], ue_root: Path) -> None:
...
def link_commands(base_command: str, intermediate_directory: Path, ue_root: Path) -> tuple[str, str, str, str]:
    link_responses = sorted(intermediate_directory.glob("*.link.rsp"))
    if len(link_responses) != 1:
        raise ValueError(
            f"Expected exactly one UBT link response in {intermediate_directory}, found {len(link_responses)}"
        )
...
def load_build_commands(metadata: dict, ue_root: Path, shared_definitions: Path) -> list[dict[str, object]]:
    base_command = metadata["baseCommand"]
    intermediate_directory = Path(metadata["intermediateDirectory"])
    cpp_command, c_command = compile_commands(
        base_command, ue_root, ubt_definition_arguments(base_command, intermediate_directory), shared_definitions
    )
    link_command, link_dependencies_prefix, link_command_suffix, archive_command = link_commands(
        base_command, intermediate_directory, ue_root
    )
    return [
        {
            "platform": "Linux",
...
def generate(files: list[UeFile], commands: list[dict[str, object]], ue_root: Path) -> str:
    ...
    lines.extend(
        [
            "};",
            "",
            "// Bootstrap command row exported by UBT's JsonExport -HMake mode for this checkout.",
            "// cppCompileCommand comes from the export's baseCommand and maps to the shared",
...
def main() -> None:
    if len(sys.argv) != 1:
        raise SystemExit("scanner.py takes no arguments. Copy it into the UE checkout root and run ./scanner.py.")

    ue_root = Path(__file__).resolve().parent
    if not (ue_root / "Engine" / "Source").is_dir():
        raise SystemExit(
            "Copy scanner.py into the UE checkout root beside Engine, then run ./scanner.py.\n"
            f"Location: {ue_root}"
        )

    output = ue_root / "hmake.cpp"
    metadata = ue_root / METADATA_FILE
    if not metadata.is_file():
        raise SystemExit(
            f"UBT HMake metadata file not found:\n  {metadata}\n"
            "scanner.py reads this file; it does not run UBT or generate metadata.\n"
            "Generate it first with Unreal Engine's build script:\n"
            f"  cd {shlex.quote(str(ue_root))}\n"
            "  ./Engine/Build/BatchFiles/Linux/Build.sh UnrealServer Linux Debug "
            "-buildubt -Mode=JsonExport -HMake\n"
            "After that command succeeds, run ./scanner.py."
        )
    try:
        exported = json.loads(metadata.read_text(encoding="utf-8-sig"))
        for field, expected in {
            ...
            if exported[field] != expected:
                raise ValueError(f"Expected HMake export {field}={expected}, got {exported[field]!r}")
        modules = {module["name"]: module for module in exported["modules"]}
        files = scan(ue_root)
        verify_ue_specifications(files, modules, ue_root)
        shared_definitions_path = ue_root / SHARED_DEFINITIONS_FILE
        shared_definitions = generate_shared_definitions(modules)
        commands = load_build_commands(exported, ue_root, shared_definitions_path)
