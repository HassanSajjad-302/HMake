...
int main()
{
    ...
    //  the default. This also has the default LibraryType to use and default configuration to build. You can chagne
    ...
    // anything. In future, when it's ready then e.g. your use case is to only allow gcc and no other compiler. You can
    // write the following code snippet. So that in case more than one compilers are detected use gcc. If gcc is not
    // detected then exit.
    /*
    ...
    // only one Package if your are using it.
    Project project;
    ...
    // exist both at a same time because you may any at any time. So, you can make two different variants. Set their
    ...
    // Note that we passed it the above variant object. So that it will initialize it's defaults from that variant
    // object which will initialize it's defaults from Cache class static members. Executable and Library are derived
